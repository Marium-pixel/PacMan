...
public:
    ...
    Pacman(int startX, int startY, int tSize)
        ...
        speed(4.0f), animation_over(false), alive(true),
        direction(RIGHT), desiredDirection(RIGHT), animation_timer(0), energizer_timer(0), lives(3)
    {
    ...
    void update(Map& map) {
        ...
        // --- 1) READ INPUT (update desiredDirection only)
        if (IsKeyDown(KEY_RIGHT)) desiredDirection = RIGHT;
        ...
        // --- 2) TEST IF DESIRED DIRECTION IS MOVABLE
        ...
        // --- 3) APPLY DESIRED DIRECTION IF POSSIBLE
        if (canMove(desiredDirection)) {
            direction = desiredDirection;
        }

        //bool isCenteredX = fabs(x - centerX) < 0.5f;
        //bool isCenteredY = fabs(y - centerY) < 0.5f;
        //bool fullyCentered = isCenteredX && isCenteredY;

        //if (fullyCentered) {
        //    // TURN ONLY IF THE DESIRED DIRECTION IS NOW POSSIBLE
        //    if (canMove(desiredDirection)) {
        //        direction = desiredDirection;
        //    }
        //}
        ...
        // --- 4) MOVE PAC-MAN IN CURRENT DIRECTION
        float dx = 0, dy = 0;
        switch (direction) {
        case RIGHT: dx = speed; break;
        case LEFT:  dx = -speed; break;
...
public:
        ...
        animation_timer(0), texture(tex)
    {
        ...
        cageX = gx;
        cageY = gy;
        gateX = gx;       // you can adjust if gate is above them
        gateY = gy - 1;   // usually one tile above
    }
    ...
    void updateReleaseState(float tileSize)
    {
        switch (releaseState)
        {
        case R_IN_CAGE:
            // Move upward toward gate
            if (position.y > gateY * tileSize) {
                position.y -= speed;
            }
            else {
        ...
        case R_EXITING_GATE:
            // Continue until the ghost is fully outside cage area
            if (position.y > (gateY - 1) * tileSize) {
                position.y -= speed;
            }
            else {
                releaseState = R_ACTIVE;
            }
            break;

        case R_ACTIVE:
            // normal chase/scatter/frightened logic
            break;
        }
    }
...
/*
void resetAfterPacmanDeath(Pacman& pac, int tileSize,
    RedGhost& red, PinkGhost& pink,
    OrangeGhost& orange, BlueGhost& blue,
    ReleaseInfo& redInfo, ReleaseInfo& pinkInfo,
    ReleaseInfo& orangeInfo, ReleaseInfo& blueInfo)
{
    pac.alive = false;
    pac.lives--;
    ...
    // Reset ghosts **position only**, leave timers intact
    auto resetGhost = [&](Ghost& g, ReleaseInfo& info)
        {
            g.position.x = g.cageX * tileSize;
            g.position.y = g.cageY * tileSize;
            info.state = R_IN_CAGE;
            g.direction = Pacman::UP;
            g.frightened_mode = 0;
        };

    resetGhost(red, redInfo);
    resetGhost(pink, pinkInfo);
    resetGhost(orange, orangeInfo);
    resetGhost(blue, blueInfo);

    // The map remains intact, so eaten pellets persist
}
 */
...
{
    // Local static to reset releaseGhost timing after a death (kept here for isolation)
    ...
    for (Ghost* g : ghosts)
    {
        ...
        if (dx < 20 && dy < 20)  // collision detected
        {
            ...
            // 2) RESET PACMAN POSITION (pellets remain eaten)
            pac.x = pac.startXPos;
            pac.y = pac.startYPos;
            pac.direction = Pacman::RIGHT;
            // If you have desiredDirection member, keep it consistent:
            // pac.desiredDirection = Pacman::RIGHT;
            pac.alive = true;

            // 3) RESET ALL GHOSTS BACK TO THE CAGE (use the same coordinates used in main)
            // IMPORTANT: these grid coordinates must match the ones you used when creating the ghosts in main().
            ...
            // 4) RESET RELEASE STATES (so releaseGhost treats them like game start)
            // Red is considered active (starts outside) if that's how you do it in main.
            redInfo.state = R_ACTIVE;
            redInfo.timer = 0;
            redInfo.justEnteredScatter = false;

            pinkInfo.state = R_IN_CAGE;
            pinkInfo.timer = 0;
            pinkInfo.justEnteredScatter = false;

            orangeInfo.state = R_IN_CAGE;
            orangeInfo.timer = 0;
            orangeInfo.justEnteredScatter = false;

            blueInfo.state = R_IN_CAGE;
            blueInfo.timer = 0;
            blueInfo.justEnteredScatter = false;

            // 5) RESET THE FRAME COUNTER USED BY releaseGhost so timings restart
            framesSinceStart = 0;
            redLeftFrame_global = -1;

            // 6) CALL releaseGhost so they will exit exactly like at start (pink/orange/blue)
            // Note: releaseGhost expects (Ghost& g, Ghost& red, Map& map, int tileSize, int framesSinceStart, ReleaseInfo& info)
            ...
            // Done — avoid handling multiple collisions in same frame
            return;
        }
    }
}
...
int main() {
    ...
    ReleaseInfo redRelease;

    // If Red starts outside, mark active
    ...
    while (!WindowShouldClose()) {
        ...
        checkPacmanGhostCollision(
            ...
            tileSize,
            redRelease,     // redInfo
            blueRelease,    // blueInfo  <-- MUST COME SECOND
            pinkRelease,    // pinkInfo
            orangeRelease,  // orangeInfo
            maze,           // Map& map
            globalFrames    // int& framesSinceStart
        );
        // -------------------- Draw --------------------
        BeginDrawing();
