...
constexpr unsigned char START_GATE_Y = 7;
constexpr unsigned char START_GATE_X = 10;
...
public:
    ...
    Pacman(int startX, int startY, int tSize)
        ...
        speed(3.5f), animation_over(false), dead(false),
...
enum PinkyReleaseState { IN_CAGE, EXITING_GATE, ACTIVE };
// -------------------- Ghost --------------------
class Ghost {
public:
    Vector2 position;
    ...
    PinkyReleaseState releaseState = IN_CAGE;
    int releaseTimer = 1;
    bool justEnteredScatter = false;
...
void chasePacman(Ghost& g, Pacman& p, Map& map, int tileSize) {
    ...
    float speed = 2.8f; // red ghost speed
    if (len > 0) {
        dx = dx / len * speed;
        dy = dy / len * speed;
        g.position.x += dx;
...
{
    // ------------------ 1️⃣ Target corner ------------------
    int targetX = 0; // top-left corner (or any reachable tile you prefer)
    int targetY = 0;
    ...
    {
        // fallback: pick nearest reachable BFS-visited neighbor
        int bestDist = INT_MAX;
        for (auto& d : dirs)
        {
            int ny = gy + d[0];
...
{
    // ------------------ 1️⃣ Calculate target 4 tiles ahead ------------------
    ...
    // ------------------ 3️⃣ Determine next cell ------------------
    std::pair<int, int> nextCell = { gy, gx }; // default: stay in place

    if (found)
    {
        // normal BFS path reconstruction
        std::pair<int, int> cur = { targetY, targetX };
        while (parent[cur.first][cur.second] != std::make_pair(gy, gx))
            cur = parent[cur.first][cur.second];
        nextCell = cur;
    ...
    {
        // BFS failed: target unreachable → pick nearest BFS-visited neighbor
        int bestDist = INT_MAX;
        ...
        for (auto& d : dirs)
        {
            ...
            if (nx >= 0 && nx < map.cols &&
                ny >= 0 && ny < map.rows &&
                !map.isWall(nx, ny) &&
                visited[ny][nx]) // must be reachable by BFS
            {
                int dist = abs(nx - targetX) + abs(ny - targetY); // Manhattan distance
                if (dist < bestDist)
                {
                    bestDist = dist;
                    nextCell = { ny, nx };
    ...
    float speed = 3; // Pinky slightly faster than red
    if (len > 0.001f)
    {
        dx = dx / len * speed;
        dy = dy / len * speed;
...
void updatePinky(Ghost& pink, Ghost& red, Pacman& pac, Map& map,  int tileSize, int pacDir, int chaseTiles, bool scatterMode) {
    // Only Pinky
    if (pink.id != 1) return;

    const int GATE_EXIT_Y = 7;      // ← FIXED COORD
    const float exitSpeed = 2.0f;

    switch (pink.releaseState)
    {
        // 1️⃣ WAIT inside cage
    case IN_CAGE:
        // Check if red left the cage
        if (red.position.y / tileSize < GATE_EXIT_Y) {
            pink.releaseTimer++;

            // 10 sec delay @ 60 FPS = 600 frames
            if (pink.releaseTimer >= 480) {
                pink.releaseState = EXITING_GATE;
            }
        }
        break;

        // 2️⃣ MOVE UP OUT OF THE GATE
    case EXITING_GATE: {
        pink.position.y -= exitSpeed;

        if (pink.position.y / tileSize <= GATE_EXIT_Y) {
            pink.releaseState = ACTIVE;
        }
        break;
    }
    ...
    case ACTIVE:
        if (scatterMode) {
            // If we just entered scatter mode, reset the next target
            if (!pink.justEnteredScatter) {
                pink.justEnteredScatter = true;  // new flag in Ghost
                // Optional: snap to center of current tile to avoid weird offsets
                pink.position.x = ((int)(pink.position.x / tileSize) * tileSize) + tileSize / 2.0f;
                pink.position.y = ((int)(pink.position.y / tileSize) * tileSize) + tileSize / 2.0f;
            }
            scatterPink(pink, map, tileSize);
        }
        else {
            pink.justEnteredScatter = false;  // reset when leaving scatter
            chasePink(pink, pac, map, tileSize, pacDir, chaseTiles);
        }
        break;
...
int main() {
    ...
    ghosts.push_back(Ghost(9, 8, 1, ghostTexture, tileSize));  // pink
    ghosts.push_back(Ghost(11, 8, 2, ghostTexture, tileSize)); // blue
    ghosts.push_back(Ghost(10, 8, 3, ghostTexture, tileSize)); // orange 
    ...
    while (!WindowShouldClose()) {
        ...
        // --- Update Pinky ---
        updatePinky(ghosts[1], ghosts[0], pac, maze, tileSize, pacDir, 4,scatterMode);
