...
void Pacman::setHardMode(bool hard) {
    isHard = hard;
    speed = isHard ? HARD_SPEED : NORMAL_SPEED;
}
...
void Pacman::checkLargePellet(Map& map) {
    if (isHard) return; // No large pellets in hard mode
...
void Pacman::updatePacMan(Map& map, CoinList& coins) {
    ...
    // Current center
    float currentCenterX = x + tileSize / 2.0f;
    float currentCenterY = y + tileSize / 2.0f;

    // Get grid alignment
    int gridX = (int)(currentCenterX / tileSize);
    int gridY = (int)(currentCenterY / tileSize);
    float gridCenterX = gridX * tileSize + tileSize / 2.0f;
    float gridCenterY = gridY * tileSize + tileSize / 2.0f;

    // Gentle snap when very close to center (helps with alignment)
    if (fabs(currentCenterX - gridCenterX) < 1.0f) {
        x = gridCenterX - tileSize / 2.0f;
    }
    if (fabs(currentCenterY - gridCenterY) < 1.0f) {
        y = gridCenterY - tileSize / 2.0f;
    }

    // Recalculate after snap
    currentCenterX = x + tileSize / 2.0f;
    currentCenterY = y + tileSize / 2.0f;

    // Check if we can move in a direction
    auto canMove = [&](Direction dir) -> bool {
        float testX = currentCenterX;
        float testY = currentCenterY;
        switch (dir) {
        ...
        return !collidesAtCenter(map, testX, testY);
        };

    // Check if current direction is blocked
    bool currentDirectionBlocked = !canMove(direction);

    // Try to change direction - ALWAYS check if desired is different, 
    // even if not stuck (this was the issue!)
    if (desiredDirection != direction) {
        bool canTurnNow = false;

        // Use lenient threshold when stuck, normal when moving
        float alignmentThreshold = currentDirectionBlocked ? (tileSize / 2.0f) : (speed * 2.0f);

        // Check if this is a perpendicular turn that requires alignment
        if ((direction == LEFT || direction == RIGHT) && (desiredDirection == UP || desiredDirection == DOWN)) {
            // Horizontal to vertical - need Y alignment
            float distY = fabs(currentCenterY - gridCenterY);
            if (distY <= alignmentThreshold && canMove(desiredDirection)) {
                y = gridCenterY - tileSize / 2.0f;
                canTurnNow = true;
            }
        }
        else if ((direction == UP || direction == DOWN) && (desiredDirection == LEFT || desiredDirection == RIGHT)) {
            // Vertical to horizontal - need X alignment
            float distX = fabs(currentCenterX - gridCenterX);
            if (distX <= alignmentThreshold && canMove(desiredDirection)) {
                x = gridCenterX - tileSize / 2.0f;
                canTurnNow = true;
            }
        }
        else if (canMove(desiredDirection)) {
            // 180-degree turn - no alignment needed
            canTurnNow = true;
        }
    ...
    // Move in the current direction (only if not blocked)
    ...
    if (speedTimer > 0) {
        speedTimer--;
        if (speedTimer == 0) speed = isHard ? HARD_SPEED : NORMAL_SPEED;
    }
