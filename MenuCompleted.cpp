...
typedef enum { STATE_MENU, STATE_LOADING , STATE_PLAYING, STATE_HIGHSCORE, STATE_HOW_TO, STATE_EXIT } GameState;  // Expand your states
...
{
    static float slideY = -100.0f;    // Start off-screen
...
int main() {
    ...
    Font titleFont = LoadFont("pacman_font.ttf");
    ...
    while (!WindowShouldClose()) {
        ...
        if (pac.lives <= 0)
        {
            ...
            gameOverTimer = 0;  // Reset for next time
            // Reset maze (re-eat pellets if needed, but since it's a reset, assume fresh)
            // If you want to reset pellets, reload the maze or add a reset function
            // Transition to loading screen
            currentState = STATE_LOADING;
            loadingFrame = 0;  // Reset loading animation
            loadPacX = 0.0f;   // Reset Pacman position in loading
        }
        EndDrawing();
        continue;
    }
