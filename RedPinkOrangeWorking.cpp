...
public:
    ...
    Direction direction; // replaced unsigned char
    ...
    Pacman(int startX, int startY, int tSize)
        ...
        direction(RIGHT), animation_timer(0), energizer_timer(0)
    {
...
public:
    ...
    int main() {
        ...
        // -------------------- Ghosts --------------------
        // Create RedGhost instance (Blinky) so we can call its update() which uses global navigation
        RedGhost red(10, 8, 0, ghostTexture, tileSize); // Blinky (red)
        PinkGhost pink(11, 8, 1, ghostTexture, tileSize);
        OrangeGhost orange(10, 9, 3, ghostTexture, tileSize);

        // Keep other ghosts (pink/blue/orange) as base Ghost objects for now
        vector<Ghost> otherGhosts;
        //otherGhosts.push_back(Ghost(9, 9, 1, ghostTexture, tileSize));  // pink
        otherGhosts.push_back(Ghost(11, 9, 2, ghostTexture, tileSize)); // blue
        //otherGhosts.push_back(Ghost(10, 9, 3, ghostTexture, tileSize)); // orange

        // Wave timer
        int waveTimer = 0;
        bool scatterMode = true; // start scattering

        // (Removed inline chase lambda — RedGhost::update calls chasePacmanToTarget / scatterToCorner.)

        // -------------------- Game Loop --------------------
        while (!WindowShouldClose()) {
            // update pacman (your implementation)
            pac.update(maze);

            // Wave timer: scatter 7s, chase 20s (same as before)
            waveTimer++;
            ...
            // -------------------- Update Red Ghost (uses global navigateToTile via wrappers) --------------------
            ...
            // (Optional) Update other ghosts later when you make classes for them.
            // For now they remain static or you can call some movement if needed.
