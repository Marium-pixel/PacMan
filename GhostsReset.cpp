...
void checkPacmanGhostCollision(Pacman& pac,
    ...
    PinkGhost& pink,
    OrangeGhost& orange,
    BlueGhost& blue, int tileSize,
    ReleaseInfo& redInfo,
    ReleaseInfo& blueInfo, ReleaseInfo& pinkInfo,
    ReleaseInfo& orangeInfo)
{
    ...
    for (Ghost* g : ghosts)
    {
        ...
        if (dx < 20 && dy < 20) // collision radius
        {
            resetAfterPacmanDeath(pac,tileSize,red,pink,orange,blue,redInfo, pinkInfo, orangeInfo, blueInfo); // handles lives, respawn, game over
        }
    }
}
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        checkPacmanGhostCollision(pac, red, pink, orange, blue, tileSize,redRelease, pinkRelease, orangeRelease, blueRelease);
        // -------------------- Draw --------------------
        BeginDrawing();
