...
void BubbleSortHighscores(vector<HighscoreEntry>& hs) {
    int n = hs.size();
    for (int i = 0; i < n - 1; i++) {
        for (int j = 0; j < n - i - 1; j++) {
            if (hs[j].score < hs[j + 1].score) {   // Sort descending
                swap(hs[j], hs[j + 1]);
            }
        }
    }
...
// -------------------- DrawHighscoreScreen (NEW FUNCTION) --------------------
    ...
    vector<HighscoreEntry>& highscores)
{
    ...
    BubbleSortHighscores(highscores);
    ...
    for (int i = 0; i < 3 && i < highscores.size(); i++) {
        string line = to_string(i + 1) + string(". ") +
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (currentState == STATE_HIGHSCORE) {
            DrawHighscoreScreen(winW, winH, titleFont, highscores);
