...
public:
    ...
    void die() {
        ...
        if (lives <= 0) {
            ...
            // Optionally: reset ghosts to cage
        }
    }
    ...
    BlueGhost& blue)
{
    ...
    for (Ghost* g : ghosts)
    {
        ...
        if (dx < 12 && dy < 12) // collision radius
        {
            pac.die(); // handles lives, respawn, game over
        }
    }
}
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        checkPacmanGhostCollision(pac, red, pink, orange, blue);
        // -------------------- Draw --------------------
        BeginDrawing();
