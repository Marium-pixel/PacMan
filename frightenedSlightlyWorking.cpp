...
// ------------------------------------------------------
    void checkLargePellet(Map& map) {
        ...
        // Check if this tile is a large pellet
            ...
            map.layout[gy][gx] == 'O')
        {
            ...
            // Start energizer mode (7 seconds at 60 FPS)
            energizer_timer = 7 * 60;
        }
    }
...
)
{
    ...
    // ----------- REAL CIRCLE COLLISION (CORRECT FIX) ----------
    for (Ghost* g : ghosts)
    {
        ...
        float ghostRadius = tileSize * 0.40f;   // same as Pac-Man mouth radius approx
        float collisionDist = pacRadius + ghostRadius;
        float collisionDistSq = collisionDist * collisionDist;

        if (distSq < collisionDistSq)   // ⭐ Proper collision detected
        {
            // ---------------------------------------
            ...
            // ---------------------------------------
            // 3) RESET ALL GHOSTS BACK TO CAGE
            // ---------------------------------------
            ...
            // ---------------------------------------
            // 5) RESET RELEASE TIMERS
            // ---------------------------------------
            framesSinceStart = 0;
            redLeftFrame_global = -1;

            // ---------------------------------------
            // 6) CALL releaseGhost TO START RELEASE CYCLE AGAIN
            // ---------------------------------------
...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        // ⭐ ADDED — decrease energizer each frame
        if (pac.energizer_timer > 0)
            pac.energizer_timer--;

        // ⭐ ADDED — check if Pac-Man eats energizer
        {
