...
// ----------------------------------------------------
// GLOBAL GHOST RETURN-TO-GATE FUNCTION (BACKTRACKING)
// ----------------------------------------------------
/*
void moveGhostToGate(Ghost& g, Map& map, int tileSize)
{
    // ONLY when ghost has been eaten
    if (g.frightened_mode != 2) return;

    const int gateX = g.eyesTargetX;
    const int gateY = g.eyesTargetY;

    float speed = 2.5f; // eyes speed

    // Convert position → grid
    int gx = (int)((g.position.x + tileSize / 2) / tileSize);
    int gy = (int)((g.position.y + tileSize / 2) / tileSize);

    // -------- MOVE HORIZONTALLY --------
    if (gx < gateX)      g.position.x += speed;
    else if (gx > gateX) g.position.x -= speed;

    // Recalculate after moving X
    gx = (int)((g.position.x + tileSize / 2) / tileSize);

    // -------- MOVE VERTICALLY --------
    if (gy < gateY)      g.position.y += speed;
    else if (gy > gateY) g.position.y -= speed;

    // Recalculate again
    gx = (int)((g.position.x + tileSize / 2) / tileSize);
    gy = (int)((g.position.y + tileSize / 2) / tileSize);

    // -------- REACHED GATE --------
    if (gx == gateX && gy == gateY)
    {
        g.frightened_mode = 0;          // back to NORMAL instantly
        g.releaseState = R_EXITING_GATE;
        g.speed = 1.0f;        // restore your original chase/scatter speed
        g.position.x = gateX * tileSize;
        g.position.y = gateY * tileSize;
    }
}
*/
    ...
    for (Ghost* g : ghosts)
    {
        ...
        if (distSq < collisionDistSq)
        {
            ...
            if (g->frightened_mode == 1) // ghost is edible
            {
                ...
                g->position.x = g->cageX * tileSize;//THESE 2 LINES SEND GHOST BACK TO GATE
                g->position.y = g->cageY * tileSize;//IN FRIGHTENED MODE AND STOP COLLISION
            ...
            const int ORANGE_CAGE_COL = 11, ORANGE_CAGE_ROW = 9;  // Changed from 10 to 11
            const int BLUE_CAGE_COL = 10, BLUE_CAGE_ROW = 9;

            red.position = { 10.f * tileSize, 8.f * tileSize };
            pink.position = { 9.f * tileSize, 9.f * tileSize };
            orange.position = { 11.f * tileSize, 9.f * tileSize };  // Changed from 10.f to 11.f
            blue.position = { 10.f * tileSize, 9.f * tileSize };     // Changed from 11.f to 10.f
...
int main() {
    ...
    OrangeGhost orange(11, 9, 3, ghostTexture, tileSize);  // Changed x from 10 to 11
    BlueGhost blue(10, 9, 2, ghostTexture, tileSize);      // Changed x from 11 to 10
