...
void Merge(vector<HighscoreEntry>& hs, int left, int mid, int right) {
    ...
    while (i < n1 && j < n2) {
        if (L[i].score >= R[j].score) {   // Descending order
            hs[k++] = L[i++];
        }
        else {
            hs[k++] = R[j++];
    ...
    const string& currentPlayer)
{
    ...
    // Sort highscores using merge sort
    if (!highscores.empty())
    ...
    for (int i = 0; i < 3 && i < highscores.size(); i++) {
        string line = to_string(i + 1) + ". " + highscores[i].name + " - " +
    ...
    // Display "You: rank name score" if player found
    if (playerRank != -1) {
        string youLine = "You: " + to_string(playerRank) + " " +
            highscores[playerRank - 1].name + " " + "-" + " " +
            to_string(highscores[playerRank - 1].score);
